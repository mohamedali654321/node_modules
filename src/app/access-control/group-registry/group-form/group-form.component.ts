...
export class GroupFormComponent implements OnInit, OnDestroy {
      ...
      .subscribe((list: PaginatedList<Group>) => {
        if (list.totalElements > 0) {
          this.notificationsService.error(this.translateService.get(this.messagePrefix + '.notification.' + notificationSection + '.failure.groupNameInUse', {
            name: this.localeService.getStringByLocale(group.name)
          }));
        }
      }));
  }
  ...
  editGroup(group: Group) {
    ...
    this.groupDataService.patch(group, operations).pipe(
      ...
      if (rd.hasSucceeded) {
        this.notificationsService.success(this.translateService.get(this.messagePrefix + '.notification.edited.success', { name: this.localeService.getStringByLocale(rd.payload.name)}));
        this.submitForm.emit(rd.payload);
      } else {
        this.notificationsService.error(this.translateService.get(this.messagePrefix + '.notification.edited.failure', { name: this.localeService.getStringByLocale(group.name ) }));
        this.cancelForm.emit();
      }
    });
  }
          ...
          if (hasValue(group.id)) {
            this.groupDataService.delete(group.id).pipe(getFirstCompletedRemoteData())
              .subscribe((rd: RemoteData<NoContent>) => {
                if (rd.hasSucceeded) {
                  this.notificationsService.success(this.translateService.get(this.messagePrefix + '.notification.deleted.success', { name: this.localeService.getStringByLocale(group.name )}));
                  this.onCancel();
                } else {
                  this.notificationsService.error(
                    this.translateService.get(this.messagePrefix + '.notification.deleted.failure.title', { name: this.localeService.getStringByLocale(group.name ) }),
