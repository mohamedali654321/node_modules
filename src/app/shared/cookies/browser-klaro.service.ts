...
export class BrowserKlaroService extends KlaroService {
   ...
   */
   addAppMessages() {
    /**kware start editing */
     const currentLocale = this.localeService.getCurrentLanguageCode() || environment.defaultLanguage;
    //const currentLocale = this.localeService.getCurrentLanguageCode() ;
    this.klaroConfig.services.forEach((app) => {
      this.klaroConfig.translations[currentLocale][app.name] = { title: this.getTitleTranslation(app.name), description: this.getDescriptionTranslation(app.name) };
      app.purposes.forEach((purpose) => {
        this.klaroConfig.translations[currentLocale].purposes[purpose] = this.getPurposeTranslation(purpose);
      });
    });
    /**kware end editing */
  }
   ...
   */
   translateConfiguration() {
    ...
    const currentLocale = this.localeService.getCurrentLanguageCode() || environment.defaultLanguage;
    //const currentLocale = this.localeService.getCurrentLanguageCode() ;
    this.translateService.setDefaultLang(currentLocale);
    this.translate(this.klaroConfig.translations[currentLocale]);
    /**kware end editing */
  }
  /**
