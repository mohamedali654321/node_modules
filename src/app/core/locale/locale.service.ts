...
import { hasValue } from '../../../app/shared/empty.util'; // kware edit
...
/*        if (isAuthenticated && isLoaded) {
   ...
   */
  // kware Edit
  // change direction if lang ia Ar
  setCurrentLanguageCode(lang?: string): void {
    ...
    window.localStorage.setItem('selectedLangCode', lang);
    }
  ...
  public refreshAfterChangeLanguage() {
   ...
   // kware edit
  // separate string by bar '|'
  getStringByLocale(value: string ){
    if (value && value.includes('|')) {
      return this.getCurrentLanguageCode() === 'ar' ? value.split('|')[1] : value.split('|')[0];
    }
    return value;
}
// end kware edit
